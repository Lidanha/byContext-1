...
import rules.{Creators, RulesTestsHelper}

class ScoreCalculatorTests extends WordSpecLike with Matchers with RulesTestsHelper with MockFactory with Creators{
  "ScoreCalculator" must {
    "a single value with a single relevant rule should be selected" in {
      val calcResult = new DefaultScoreCalculator().calculate(emptyCTX, Array(PossibleValue("v", Some(relevant))))
    ...
    "a single value with a single neutral rule should be selected" in {
      val calcResult = new DefaultScoreCalculator().calculate(emptyCTX, Array(PossibleValue("v", Some(neutral))))
    ...
    "a single value without rules should be selected" in {
      val calcResult = new DefaultScoreCalculator().calculate(emptyCTX, Array(PossibleValue("v", None)))
    ...
    "an empty list of values should return an empty calc result" in {
      val calcResult = new DefaultScoreCalculator().calculate(emptyCTX, Array.empty[PossibleValue])

      calcResult.size should be (0)
    }
    "a single value with a not relevant rule is not selected" in {
      val calcResult = new DefaultScoreCalculator().calculate(emptyCTX, Array(PossibleValue("a",Some(notRelevant))))

      calcResult.size should be (0)
    }
    "two values one has a relevant rule and the other with a not relevant rule selects the value with the relevant rule" in {
      val calcResult = new DefaultScoreCalculator().calculate(emptyCTX,
